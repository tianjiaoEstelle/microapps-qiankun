...
import ResumeSingleForm from './ResumeSingleForm.vue'
//具有多选项控件枚举
...
export default class ResumeFormContainer extends Vue.with(Props) {
  ...
  get subAvatar() {
    let srcStr = ''
    srcStr = this.form?.basic?.avatar
    if (srcStr === undefined || srcStr === '') {
      const sex = this.form?.basic?.sex
      if (sex === '女') {
  ...
  ruleFormRef = ref()
  dictIds: any = [] //字典项id集合
  dictMap: any = {} //所有字典项对象-单选、多选、下拉框选项
  formList: any = [] //简历字段集合-用于绘制表单组件
  form: any = {} //简历表单数据结构
  lastForm: any = {} //简历表单数据结构-保存之前数据，用于返回恢复
  rules: any = {} //简历表单校验规则
  setHeight: any = null
  //根据简历模板id获取简历模板字段
  ...
  fillDictIds(fieldsMap: any) {
    ...
    fieldsMap.forEach((element: any) => {
      if (element.fieldsList.length > 0) {
        element.fieldsList.forEach((item: any) => {
